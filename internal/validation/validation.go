...
func HandleValidationErrors(err error) gin.H {
	if validationError, ok := err.(validator.ValidationErrors); ok {
		...
		for _, e := range validationError {
			...
			case "search":
				errors[fieldPath] = fmt.Sprintf("%s must contain only lowercase letters, uppercase letters, numbers, and spaces", fieldPath)
			case "email":
			...
			case "email_advanced":
				errors[fieldPath] = fmt.Sprintf("%s is in the banned list", fieldPath)
			case "password_strong":
				errors[fieldPath] = fmt.Sprintf("%s must be at least 8 characters long and contain (lowercase letters, uppercase letters, numbers, and special characters)", fieldPath)
			case "file_ext":
				allowedValues := strings.Join(strings.Split(e.Param(), " "), ",")
